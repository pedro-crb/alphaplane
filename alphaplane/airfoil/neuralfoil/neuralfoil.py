...
def softmax(
        *args: Union[float, np.ndarray],
        softness: float = np.nan,
        hardness: float = np.nan,
) -> Union[float, np.ndarray]:
    n_specified_arguments = (not np.isnan(hardness)) + (not np.isnan(softness))
    ...
    if not np.isnan(hardness):
        softness = 1 / hardness

    if np.any(softness <= 0):
        if not np.isnan(softness):
    ...
    args = tuple([arg / softness for arg in args])
    mn = args[0]
    mx = args[0]
    for arg in args[1:]:
        mn = np.fmin(mn, arg)
    ...
    out = mx + np.log(sum([np.exp(np.maximum(array - mx, -500)) for array in args]))
    out = out * softness
    return out
...
def softmin(
    ...
    return -softmax(*[-arg for arg in args], softness=softness, hardness=hardness)
